...
# python test.py --swin_type base --dataset refsegrs --resume ./model_best_FIANet.pth --split test --window12 --img_size 480 --num_tmem 1
...
def evaluate(model, data_loader, bert_model, device):
    ...
    with torch.no_grad():
        ...
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            image, target, sentences, attentions, target_masks, position_masks, save_prefix, pic_name = data
            ...
            for j in range(sentences.size(0)):
                ...
                # save_path = "/media/ssd/zhangyijie/FIANet-master/FIANet-master/val/pic"
                # # 检查路径是否存在，如果不存在则创建
                ...
                # save_mask_as_jpg(output_mask, save_path)
                ...
                save_pred_targ_results(output_mask, target, image, save_path, pic_name)
                ...
                # mask = output_mask * 255
                # cv2.imwrite(os.path.join(save_dir, save_prefix[0] + "_" + str(this_iou) + "_pred.png"), mask[0, :, :])
...
def save_pred_targ_results(output_mask, target, image, save_path, pic_name):
    "leisen: show the predict and target outcomes"
    pred = output_mask[0, :, :]
    targ = target[0, :, :]
    # pred = np.uint8(pred * 255)
    # targ = np.uint8(targ * 255)
    # cv2.imwrite("pred.png", pred)
    # cv2.imwrite("targ.png", targ)
    ...
    # print(pic_name)
    number = pic_name[0].split('/')[-1].split('.')[0]  # 分割路径和扩展名
    # print(number)  # 输出: 4629

    # Save original image (added this part)
    cv2.imwrite(f"{save_path}_input_{number}.png", cv2.cvtColor(im, cv2.COLOR_RGB2BGR))
